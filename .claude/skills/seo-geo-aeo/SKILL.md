...
This project runs in a local Claude Code CLI environment (not a hosted sandbox), so outputs go under a `reports/` folder at the project root rather than a `/sessions/.../mnt/outputs` path, and there is no `present_files` tool or `computer://` link scheme — deliver plain local file paths instead.
...
**Then immediately write and run the full report script in the next tool call — do not pause, do not add intermediate steps.** Write the complete JS to a file (e.g. `reports/generate-audit.js`) and execute it in one shot.
...
- Line 1: Audit date
- Line 2: "Generated by the seo-geo-aeo Claude Code skill"
...
**Footer:** Page number right-aligned. Separated by a gray top border.
...
PDF conversion needs LibreOffice's `soffice` binary. Check for it first, and skip PDF generation gracefully (deliver the .docx only, and say why) if it isn't installed rather than failing the whole report:

```bash
command -v soffice >/dev/null 2>&1 && soffice --headless --convert-to pdf reports/seo-audit-[domain]-[date].docx --outdir reports/ || echo "soffice not found — skipping PDF, .docx still delivered"
```

If the user wants PDF output and `soffice` isn't present, mention it can be installed with `brew install --cask libreoffice`.

### Deliver to the user

Report the local paths directly — this environment has no `present_files` tool or `computer://` link scheme:

```
Your audit report is ready:
- reports/seo-audit-[domain]-[date].docx
- reports/seo-audit-[domain]-[date].pdf (if generated)
```
